...
(:action schedule_middle_sub_task
    ...
    :precondition (and  (not (scheduled ?task))
                        (exists (?ts - time_slice)
                        (tasks_separated_by ?task ?time_slice ?ts)
                        )
                        ...
                        (not (exists (?ts - time_slice)
                                     (exists (?end_ts - time_slice)
                                        (and (tasks_separated_by ?task ?time_slice ?end_ts) (in_between ?time_slice ?end_ts ?ts) (ts_occupied ?m ?ts))
                                     )
                        ))
    ...
    :effect (and (scheduled ?task) 
                 ...
                 (forall (?end_ts - time_slice)
                    (when (tasks_separated_by ?task ?time_slice ?end_ts)
                        (ts_occupied ?m ?end_ts))
                 )
                (forall (?end_ts - time_slice)
                    (when (tasks_separated_by ?task ?time_slice ?end_ts)
                    (forall (?ts_next - time_slice) 
                    (when (ts_precedes ?end_ts ?ts_next)
                ...
                (forall (?end_ts - time_slice)
                    ...
                    (when (in_between ?time_slice ?end_ts ?ts)
                          (ts_occupied ?m ?ts))
                 ) )
                 )
